...
#include <OpenGL/gl.h>
#else
#include "GL/gl.h"           //i change <> to ""
#endif

#ifdef WIN32
# include "GL/wglext.h"      //i change <> to ""
#endif
...
#else
#include "GL/glext.h" //i change <> to ""
#endif
...
static int ExtensionExists(const char* extName, const char* sysExts){
    ...
    if (0 == strcmp(extName, "GL_VERSION_1_2")) {
        ...
        if (strstr(version, "1.0") == version || strstr(version, "1.1") == version) {return GL_FALSE;} else {return GL_TRUE;}
    }
    if (0 == strcmp(extName, "GL_VERSION_1_3")) {
        const char *version = (const char*)glGetString(GL_VERSION);
        if (strstr(version, "1.0") == version ||
            strstr(version, "1.1") == version ||
            strstr(version, "1.2") == version) {return GL_FALSE;} else {return GL_TRUE;}
    }
    if (0 == strcmp(extName, "GL_VERSION_1_4")) {
        ...
        if (strstr(version, "1.0") == version ||
            ...
            strstr(version, "1.3") == version) {return GL_FALSE; } else {return GL_TRUE;}
    }
    if (0 == strcmp(extName, "GL_VERSION_1_5")) {
        ...
        if (strstr(version, "1.0") == version ||
            ...
            strstr(version, "1.4") == version) {return GL_FALSE; } else { return GL_TRUE;}
    }
    if (strstr(sysExts, padExtName)) {free(padExtName);return GL_TRUE;} else { free(padExtName);return GL_FALSE;}
}
...
int glh_init_extensions(const char *origReqExts){                 //return 1;
    ...
    if (NULL == sysExts) {
        ...
        size_t sysExtsLen = strlen(extensions);//AV if createwindow not EX and not done register wc
    ...
    if (NULL == origReqExts) return GL_TRUE;
...
int glh_extension_supported(const char *extension){
    ...
    const GLubyte *start;
    GLubyte *were, *terminator;
    // Extension names should not have spaces.
    were = (GLubyte *) strchr(extension, ' ');
    if (were || *extension == '\0')return 0;
    ...
    start = extensions;
    for (;;){
        were = (GLubyte *) strstr((const char *) start, extension);
        if (!were)break;
        terminator = were + strlen(extension);
        if (were == start || *(were - 1) == ' ') {
